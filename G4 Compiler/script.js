...
$("#startStop").click(function() {
  if ($("#startStop").text() === "Start") {
      ...
      // Automate the DFA reading process
      var interval = setInterval(function() {
          if (nextSymbolIndex < inputString.length) {
              ...
              checkFinalState(); // Check the final state of the DFA
              $("#startStop").text("Stop");
          }
      }, 500); // Adjust the interval time as needed (500ms here)
  } else {
      stopSimulation();
  }
});
...
function checkFinalState() {
  var isAccepting = currentStates.some(state => noam.util.contains(automaton.acceptingStates, state));
  if (isAccepting) {
      $("#result").text("Valid String").css("color", "rgb(57, 199, 93)"); // Green
  } else {
      $("#result").text("Invalid String").css("color", "rgb(243, 61, 61)"); // Red
  }
}
...
function onInputStringChange() {
  $("#result").text(""); // Clear previous result message
